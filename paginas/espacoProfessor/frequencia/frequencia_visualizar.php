    ...
    <main class="container mt-4">
        ...
        <div class="card mb-4 p-5">
            ...
            <table class="table table-bordered ">
                ...
                <tbody>
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM `corvo_usuarios_turma` JOIN corvo_usuarios ON corvo_usuarios.id = `corvo_usuarios_turma`.usuario WHERE `corvo_usuarios_turma`.turma = :turma");
                        $stmt->bindParam(":turma", $_GET["turma"]);
                        $stmt->execute();
                        ...
                        foreach ($alunos as $aluno) {
                            ...
                            $stmt = $conn->prepare("SELECT COUNT(*) AS faltas FROM corvo_presencas JOIN corvo_aulas ON corvo_aulas.siglaTurma = :turma WHERE aluno = :aluno AND presenca = 0 GROUP BY aluno, turma");
...
console.log(<?php var_dump($turma); ?>);
        ...
        var frequencia = {
            atribuir: function() {
                ...
                checkboxes.forEach(function(checkbox) {
                    if (checkbox.checked) {
                        frequencia.push(checkbox.value); // matriculas com frequência
                    }
                });
